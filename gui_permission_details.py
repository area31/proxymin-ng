...
def print_confirmation_page(category, id, action, group=''):
...
def save_input_data(category, id, action, url_list, url_status_list, ssl, default):
    iterator = objectiterator.ObjectIterator(category)
    object = iterator.get_object(id)  
...
def print_details_form(category, action, id, group=''):    
    ...
    if(category == CATEGORY_USERS or category == CATEGORY_HOSTS):
        ...
        html_string += '<input type="hidden" name="group" value="%s">%s\n' % (group, group)
        html_string += '</td></tr>\n'        
        html_string += '<tr class = "%s">' % (row_format.get_next_row_class())
        html_string += '<th/><td align=left colspan=%i>%s</td>\n' % (colspan, _('these rules have priority over the rules of the group'))
        if(action == ACTION_EDIT_HTTP_PERMISSIONS or action == ACTION_EDIT_FTP_PERMISSIONS):
        ...
        if(action == ACTION_EDIT_HTTP_PERMISSIONS or action == ACTION_EDIT_FTP_PERMISSIONS):
            html_string += '<input type="radio" name="default" value="%s" %s>%s\n' % (ALLOWED, status_allowed, _('allowed'))
            html_string += '<input type="radio" name="default" value="%s" %s>%s</td>\n</tr>\n' % (DENIED, status_denied, _('denied'))
        else:
            html_string += '%s</td></tr>\n' % (status_text)
        html_string += '<tr class = "%s">\n' % (row_format.get_next_row_class())
        html_string += '<th/><td align=left><nobr>%s</nobr></td>\n</tr>\n' % (_('the URLs below are exceptions to the Default'))
    index = 1
    if(category == CATEGORY_GROUPS):   
        if(action == ACTION_EDIT_HTTP_PERMISSIONS or action == ACTION_SAVE_HTTP_PERMISSIONS):
    ...
    for url in url_list:
        html_string += '<tr class = "%s"><th align=right><nobr>URL(%i):</nobr></th>\n' % (row_format.get_next_row_class(), index)
        if(action == ACTION_EDIT_HTTP_PERMISSIONS or action == ACTION_EDIT_FTP_PERMISSIONS):
            html_string += '<td align=left><input type="text" name="url_list_%i" value="%s" class="url"></td>\n' % (index, url)
        else:
        ...
        if(category != CATEGORY_GROUPS):
            ...
            if(action == ACTION_EDIT_HTTP_PERMISSIONS or action == ACTION_EDIT_FTP_PERMISSIONS):
                html_string += '<td align=center><input type="radio" name="url_status_list_%i" value="allowed"%s>\n' % (index, status_allowed)
                html_string += '<td align=center><input type="radio" name="url_status_list_%i" value="denied"%s>\n' % (index, status_denied)
            else:
    ...
    if(action == ACTION_EDIT_HTTP_PERMISSIONS or action == ACTION_EDIT_FTP_PERMISSIONS):
        ...
        for i in range(row_number):
            ...
            html_string += '<td align=left><input type="text" name="url_list_%i" value="" class="url"></td>\n' % (index)
            if(category == CATEGORY_USERS or category == CATEGORY_HOSTS):   
                html_string += '<td align=center><input type="radio" name="url_status_list_%i" value="allowed"></td>\n' % (index)
                html_string += '<td align=center><input type="radio" name="url_status_list_%i" value="denied" checked></td>\n' % (index)
            html_string += '</tr>\n'
            index = index + 1
    if(action == ACTION_EDIT_HTTP_PERMISSIONS or action == ACTION_SAVE_HTTP_PERMISSIONS):
...
#        status_msn_disabled = ''
        ...
        if(category != CATEGORY_GROUPS):
            if(action == ACTION_EDIT_HTTP_PERMISSIONS):    
                html_string += '<input type="radio" name="ssl" value="%s"%s>%s&nbsp;\n' % (SSL_FROM_GROUP, status_ssl_from_group, _('as group'))
        if(action == ACTION_EDIT_HTTP_PERMISSIONS):   
    ...
    if(action == ACTION_EDIT_HTTP_PERMISSIONS or action == ACTION_EDIT_FTP_PERMISSIONS):
        ...
        html_string += '<input type="button" value="%s" onClick="opener.location.reload();opener.focus();self.close();">\n' % (_('close window'))
    html_string += '</td></tr>\n</table>\n'
