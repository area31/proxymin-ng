...
def create_acl(id, category, urls, protocol, permission=NONE):
    # category types
    # g = group
    # h = host
    # u = user
    ...
    if protocol == PROTOCOL_HTTP:
    	...
    	nameoffile = config.get("files","squid_cfg_dir") + object_string + '_h'
    	#fim nome do arquivo
    else:
        result += 'f'
        #Nome do arquivo a ser gerado para salvar os sites
        nameoffile = config.get("files","squid_cfg_dir") + object_string + '_f'
        #fim nome do arquivo
    # permission types
    # a = allowed
    # d = denied
...
def create_http_access(id, category):
    ...
    if object.is_enabled():
        ...
        if len(group.get_denied_extensions_list()) > 0:
            result += 'http_access deny ' + object_string + ' g_' + object.get_group() + '_e\n'
        # denied URLs for user/host
        if not is_denied_urls_empty(object, PROTOCOL_HTTP):
            result += 'http_access deny ' + object_string + ' HTTP http_ports ' + object_string + '_h_d\n'
            if is_ssl_enabled(object):
                result += 'http_access deny ' + object_string + ' CONNECT SSL_ports ' + object_string + '_h_d\n'
#
        if not is_denied_urls_empty(object, PROTOCOL_FTP):
            result += 'http_access deny ' + object_string + ' FTP ftp_ports ' + object_string + '_f_d\n'
        # allowed URLs for user/host
        if not is_allowed_urls_empty(object, PROTOCOL_HTTP):
            result += 'http_access allow ' + object_string + ' HTTP http_ports ' + object_string + '_h_a\n'
            if is_ssl_enabled(object):
                result += 'http_access allow ' + object_string + ' CONNECT SSL_ports ' + object_string + '_h_a\n'

        if not is_allowed_urls_empty(object, PROTOCOL_FTP):
            result += 'http_access allow ' + object_string + ' FTP ftp_ports ' + object_string + '_f_a\n'
		...
		b = dbaccess.DBAccess()
        	...
        	if group2.get_http_permissions().is_msn_enabled():
	               	result += 'http_access allow ' + object_string + ' CONNECT MSNICQGTALK_ports\n'
			result += 'http_access allow ' + object_string + ' MSNICQGTALK_sites\n'
			result += 'http_access allow ' + object_string + ' MSN_mime'
	               	result += '\n'
		else:
			result += 'http_access deny ' + object_string + ' CONNECT MSNICQGTALK_ports\n'
			result += 'http_access deny ' + object_string + ' MSNICQGTALK_sites\n'
			result += 'http_access deny ' + object_string + ' MSN_mime'
	               	result += '\n'
	else:
		if(object.is_msn_enabled()):
			...
			result += 'http_access deny ' + object_string + ' MSN_mime'
               		result += '\n'
	# URLs for group
        if(len(group.get_http_permissions().get_url_list()) > 0 or group.get_http_permissions().is_default_allow()):
            result += 'http_access allow ' + object_string + ' HTTP http_ports'
            ...
            result += '\n'
            if is_ssl_enabled(object):    
                result += 'http_access allow ' + object_string + ' CONNECT SSL_ports'
                if len(group.get_http_permissions().get_url_list()) > 0:
                    if group.get_http_permissions().is_default_allow():
                        result += ' !'
                    else:
...
#               	result += '\n'
        ...
        if(len(group.get_ftp_permissions().get_url_list()) > 0 or group.get_ftp_permissions().is_default_allow()):
            result += 'http_access allow ' + object_string + ' FTP ftp_ports'
...
def bandwidth_limit_rule(pool, obj, bandwidth_limit=None):
    acl_definition = {
        'User': lambda: 'u_' + obj.get_uid(),
        'Host': lambda: 'h_' + obj.get_ip().replace('.','_')
    }
    ...
    return 'delay_class ' + pool + ' 2\n' + \
           'delay_parameters ' + pool + ' -1/-1 ' + limit_bytes + '/' + limit_bytes + '\n' + \
...
def update():
    ...
    acl_bandwidth_rules = []
    http_access = ''
    acl_access = '# desativado no squid 3 #acl all src 0.0.0.0/0.0.0.0\n'
    ...
    for group_id in a.get_group_list():
        ...
        for url in group.get_http_permissions().get_url_list():
            urls.append(url)
        if len(urls) > 0:
            acl_group_rules += create_acl(group_id, CATEGORY_GROUPS, urls, PROTOCOL_HTTP)
        urls = []
        for url in group.get_ftp_permissions().get_url_list():
            urls.append(url)
        if len(urls) > 0:
            acl_group_rules += create_acl(group_id, CATEGORY_GROUPS, urls, PROTOCOL_FTP)
        # acl for extensions
    ...
    for ip in a.get_host_list():
        host = a.get_host(ip)
        acl_hosts += 'acl h_' + string.replace(ip,'.','_') + ' src ' + ip + '/32\n'        
        if host.is_enabled():
            urls_allowed = []
            urls_denied = []
            ...
            if len(urls_denied) > 0:
                acl_host_rules += create_acl(ip, CATEGORY_HOSTS, urls_denied, PROTOCOL_HTTP, DENIED)
            if len(urls_allowed) > 0:
                acl_host_rules += create_acl(ip, CATEGORY_HOSTS, urls_allowed, PROTOCOL_HTTP, ALLOWED)
            urls_allowed = []
            urls_denied = []
            for url in host.get_ftp_url_list():
                if host.is_ftp_url_allowed(url):
                    ...
                    urls_denied.append(url)
            if len(urls_denied) > 0:
                acl_host_rules += create_acl(ip, CATEGORY_HOSTS, urls_denied, PROTOCOL_FTP, DENIED)
            if len(urls_allowed) > 0:
                acl_host_rules += create_acl(ip, CATEGORY_HOSTS, urls_allowed, PROTOCOL_FTP, ALLOWED)
        ...
        if user.is_enabled():
            ...
            for url in user.get_http_url_list():
                if user.is_http_url_allowed(url):
                    ...
                    urls_denied.append(url)
            if len(urls_denied) > 0:
                acl_user_rules += create_acl(uid, CATEGORY_USERS, urls_denied, PROTOCOL_HTTP, DENIED)
            if len(urls_allowed) > 0:
                acl_user_rules += create_acl(uid, CATEGORY_USERS, urls_allowed, PROTOCOL_HTTP, ALLOWED)
            urls_allowed = []
            urls_denied = []
            for url in user.get_ftp_url_list():
                if user.is_ftp_url_allowed(url):
                    ...
                    urls_denied.append(url)
            if len(urls_denied) > 0:
                acl_user_rules += create_acl(uid, CATEGORY_USERS, urls_denied, PROTOCOL_FTP, DENIED)
            if len(urls_allowed) > 0:
                acl_user_rules += create_acl(uid, CATEGORY_USERS, urls_allowed, PROTOCOL_FTP, ALLOWED)
    ...
    for uid in a.get_user_list():
        ...
        if not user.is_bandwidth_limit_disabled():
            ...
            acl_bandwidth_rules.append(bandwidth_limit_rule(acl_bandwidth_pools, user))

    for ip in a.get_host_list():
        host = a.get_host(ip)
        if not host.is_bandwidth_limit_disabled():
            acl_bandwidth_pools += 1
            acl_bandwidth_rules.append(bandwidth_limit_rule(acl_bandwidth_pools, host))
        ...
        if not group.is_bandwidth_limit_disabled(): 
            for uid in a.get_group_member_users(gid):
                ...
                if user.is_bandwidth_limit_disabled():
                    acl_bandwidth_pools += 1
                    acl_bandwidth_rules.append(bandwidth_limit_rule(acl_bandwidth_pools, user, group.bandwidth_limit))
            for ip in a.get_group_member_hosts(gid):
                host = a.get_host(ip)
                if host.is_bandwidth_limit_disabled():
                    acl_bandwidth_pools += 1
                    acl_bandwidth_rules.append(bandwidth_limit_rule(acl_bandwidth_pools, host, group.bandwidth_limit))
