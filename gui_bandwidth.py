...
def print_details_form(id, category, action):
    ...
    if (action == ACTION_EDIT_BANDWIDTH):
        html_string += '<input type="text" name="bandwidth_limit" value="%s">' % (object.bandwidth_limit)
    else:
        html_string += '%s' % (object.bandwidth_limit)

    if (action == ACTION_SAVE_BANDWIDTH and object.is_bandwidth_limit_disabled()):
        html_string += ' %s\n' % (_('default'))
    else:
        html_string += ' %s\n' % (_('in kbytes'))
    ...
    if (category == CATEGORY_GROUPS):
        html_string += '- O limite é para cada usuário/host do grupo.<br/>'
    html_string += '- O limite do usuário/host prevalece sobre o do grupo.'
