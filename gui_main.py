...
def alphabet(category, group=None):
    print ('<center>')
    print ('<a href="%s">%s</a>' % (os.environ['SCRIPT_NAME']
                                   + format_post_vars(category, None,
                                   group), _('All')))
    print ('| <a href="%s">0-9</a>' % (os.environ['SCRIPT_NAME']
            + format_post_vars(category, string.digits, group)))
    for i in range(0, 26):
        print ('| <a href="%s">%s</a>' % (os.environ['SCRIPT_NAME']
                + format_post_vars(category, string.ascii_lowercase[i],
...
def print_group_selector(category, letter, default_group):
    ...
    html_string += \
        '<table cellspacing="0" cellpadding="0" border=0"><tr><td><b>%s&nbsp;</b></td>\n' \
        % _('Group')
    html_string += '<td><select name="group_selection"'
    html_string += \
        ' onChange="go(this.form.group_selection.options[this.form.group_selection.options.selectedIndex].value)">\n'

    # create select box, "option value" is the complete target link

    html_string += '<option value="%s"' % (os.environ['SCRIPT_NAME']
            + format_post_vars(category, letter, None))
    if default_group == None:
        html_string += ' selected'
    html_string += '>%s</option>\n' % _('All')
    for g in dba.get_group_list():
        html_string += '<option value="%s"' \
            % format_post_vars(category, letter, g)
        if default_group == g:
            html_string += ' selected'
        html_string += '>%s</option>\n' % g
    html_string += '</select>\n'
...
#            html_string += '<img src="%s?%s=%s">' % (IMAGE_SCRIPT,
 #                  IMAGE_GET_VARIABLE, IMAGE_BALL_RED)
        ...
        html_string += '<td>\n'
        html_string += \
            '<A href="%s?action=%s&id=%s&category=%s" target="%s"' \
            % (os.environ['SCRIPT_NAME'], ACTION_EDIT, id, category,
               WINDOW_DETAILS)
        html_string += 'onClick=popup("%s")>%s</a>&nbsp;\n' \
            % (WINDOW_DETAILS, _('Details'))
        html_string += \
            '<A href="%s?action=%s&id=%s&category=%s" target="%s"' \
            % (os.environ['SCRIPT_NAME'], ACTION_EDIT_HTTP_PERMISSIONS,
               id, category, WINDOW_DETAILS)
        html_string += 'onClick=popup("%s")>HTTP</a>&nbsp;\n' \
            % WINDOW_DETAILS
        html_string += \
            '<A href="%s?action=%s&id=%s&category=%s" target="%s"' \
            % (os.environ['SCRIPT_NAME'], ACTION_EDIT_FTP_PERMISSIONS,
               id, category, WINDOW_DETAILS)
        html_string += 'onClick=popup("%s")>FTP</a>\n' % WINDOW_DETAILS
        if category == CATEGORY_GROUPS:
            html_string += '<a href="%s?action=%s&id=%s" target="%s"' \
                % (os.environ['SCRIPT_NAME'], ACTION_EDIT_EXTENSIONS,
                   id, WINDOW_DETAILS)
            html_string += 'onClick=popup("%s")>%s</A>\n' \
    ...
    # Add msn menu - for all categorys
        ...
        html_string += \
            '<a href="%s?action=%s&id=%s&category=%s" target="%s"' \
            % (os.environ['SCRIPT_NAME'], ACTION_EDIT_MSN, id,
               category, WINDOW_DETAILS)
        html_string += 'onClick=popup("%s")>%s</A>\n' \
        ...
        html_string += \
            '<a href="%s?action=%s&id=%s&category=%s" target="%s"' \
            % (os.environ['SCRIPT_NAME'], ACTION_EDIT_BANDWIDTH, id,
               category, WINDOW_DETAILS)
        html_string += 'onClick=popup("%s")>%s</A>\n' \
    ...
    # End msn menu
        ...
        html_string += \
            '<td><a href="%s?action=%s&id=%s&category=%s" target="%s"' \
            % (os.environ['SCRIPT_NAME'], ACTION_DELETE, id, category,
               WINDOW_DETAILS)
        html_string += 'onClick=popup("%s")>%s</a></td>\n' \
            % (WINDOW_DETAILS, _('delete'))
        html_string += '</tr>\n'
        i = i + 1
    html_string += '<tr><th colspan=%s align=left>&nbsp;</th>' \
        % colspan_footer
    html_string += \
        '<th colspan=2 align=center> <A href=%s?action=%s&category=%s target="%s"' \
        % (os.environ['SCRIPT_NAME'], ACTION_CREATE, category,
           WINDOW_DETAILS)
