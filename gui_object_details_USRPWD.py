...
def validate_input_data(
    ...
    # id only must contain alphanumeric characters and '_'
        ...
        error_message = id_name + ' ' \
            + _('only alphanumeric charactes and "_" are allowed')
    ...
    elif category == CATEGORY_USERS and action == ACTION_SAVE_EDIT \
        and not (gui_misc.is_alphanum(passwd1) or passwd1
                 == PASSWORD_UNCHANGED):

        error_message = \
            _('ass password only alpanumeric characters and "_" are allowed'
              )
...
def save_input_data(
    ...
    object = iterator.get_object(id)
    if category == CATEGORY_USERS:
        if passwd != PASSWORD_UNCHANGED:
            object.set_password(passwd)
...
def print_details_form(
    ...
    if action == ACTION_CREATE:
        html_string += \
            '<td class = "%s" align=left><input type="text" name="id"' \
            % row_format.get_next_row_class()
        html_string += ' value="%s"></td></tr>\n' % id
    else:

          # uid can only be changed while creation

        html_string += \
            '<td class = "%s" align=left><input type="hidden" name="id"' \
            % row_format.get_next_row_class()
        html_string += ' value="%s"><nobr>%s</nobr></td></tr>\n' % (id,
                id)
    html_string += '<tr><th align=right>%s</th>\n' % _('Description')
    if action == ACTION_CREATE or action == ACTION_EDIT:
        html_string += '<td class = "%s" align=left><input type="text"' \
            % row_format.get_next_row_class()
        html_string += \
            'name="description" value="%s" class="description"></td></tr>\n' \
            % description
    else:
        html_string += \
            '<td class = "%s" align=left><nobr>%s</nobr></td></tr>\n' \
            % (row_format.get_next_row_class(), description)
    if category == CATEGORY_USERS or category == CATEGORY_HOSTS:
        html_string += '<tr><th align=right>%s</th>\n' % _('Group')
        html_string += '<td class = "%s" align=left>' \
            % row_format.get_next_row_class()
        if action == ACTION_CREATE or action == ACTION_EDIT:
    ...
    if category == CATEGORY_USERS:
        if action == ACTION_CREATE or action == ACTION_EDIT:
            ...
            html_string += '<tr><th align=right>%s</th>\n' \
                % _('Password')
            html_string += \
                '<td class = "%s" align=left><input type="password" name="passwd1" value="%s" size="9"></td></tr>\n' \
                % (row_format.get_next_row_class(), password)
            html_string += '<tr><th align=right>%s</th>\n' \
                % _('Password')
            html_string += \
                '<td class = "%s" align=left><input type="password" name="passwd2" value="%s" size="9"></td></tr>\n' \
                % (row_format.get_next_row_class(), password)
    if category == CATEGORY_USERS or category == CATEGORY_HOSTS:
...
'''
    ...
    if action == ACTION_CREATE or action == ACTION_EDIT or action \
        == ACTION_DELETE:
        if action == ACTION_CREATE or action == ACTION_EDIT:
            button_label = _('save')
        else:
        ...
        html_string += '<input type="submit" name="send" value="%s">\n' \
            % button_label
        html_string += \
            '''<input type="button" value="%s" onClick="window.close()"></td>
</tr>
''' \
            % _('cancel')
    else:
        html_string += '<input type="button" value="%s"' \
            % _('close window')
        html_string += \
            'onClick="opener.location.reload();opener.focus();self.close();">\n'
    html_string += '</table>\n'
