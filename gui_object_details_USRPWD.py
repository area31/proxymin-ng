...
def validate_input_data(category, action, id, passwd1, passwd2):
...
def save_input_data(category, id, description, group = '', active = FALSE, passwd = ''):
    iterator = objectiterator.ObjectIterator(category)
    ...
    if(category == CATEGORY_USERS):
        if(action == ACTION_CREATE or action == ACTION_EDIT):
            if(action == ACTION_EDIT):
                ...
                password = ''
            html_string += '<tr><th align=right>%s</th>\n' % (_('Password'))
            html_string += '<td class = "%s" align=left><input type="password" name="passwd1" value="%s" size="9"></td></tr>\n' \
                           % (row_format.get_next_row_class(), password)
            html_string += '<tr><th align=right>%s</th>\n' % (_('Password'))
            html_string += '<td class = "%s" align=left><input type="password" name="passwd2" value="%s" size="9"></td></tr>\n' \
                           % (row_format.get_next_row_class(), password)
    if(category == CATEGORY_USERS or category == CATEGORY_HOSTS):   
